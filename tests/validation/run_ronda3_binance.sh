...
# Ronda 3 - Binance (120 velas)
# - Ejecuta demo trading por ~120 velas (modo demo/testnet configurado)
...
echo "[ronda3] Ejecutando demo trading (120 velas)"
./.venv/bin/python main.py --mode=demo --exchange=binance --player=paroli --symbol=LTC/USDT:USDT --interval=1m --max-candles=120 || true

# Intenta detectar el último demo log generado
DEMO_LOG=$(ls -t ${DEMO_LOGS_DIR}/demo_*.json 2>/dev/null | head -n1 || true)
if [ -z "$DEMO_LOG" ]; then
  echo "[ronda3] No se encontró demo log en ${DEMO_LOGS_DIR}. Continúo, pero revisa la ejecución demo."
else
...
PY
  ...
  if [ -n "$TIMESTAMPS" ]; then
    ...
    echo "[ronda3] No se encontraron timestamps en el log, usando período por defecto"
...
echo "[ronda3] Descargando datos históricos de Binance → ${HIST_DATA}"
python tests/validation/download_historical_data.py --exchange binance --symbol LTC/USDT:USDT --interval 1m --start "$START_DATE" --end "$END_DATE" --output ${HIST_DATA} || echo "[ronda3] download script devolvió error; revisar tests/validation/download_historical_data.py"

# 3.1) Filtrar CSV para que contenga SOLO las velas que el demo procesó
if [ -n "$DEMO_LOG" ] && [ -f "$DEMO_LOG" ] && [ -f "${HIST_DATA}" ]; then
  echo "[ronda3] Filtrando CSV para coincidir con velas procesadas en demo"
...
PY
  ...
  echo "[ronda3] Balance inicial detectado: $INITIAL_BALANCE"
else
  INITIAL_BALANCE=10000
  echo "[ronda3] Usando balance por defecto: $INITIAL_BALANCE"
fi

# 4) Ejecutar backtest con los datos descargados y el mismo balance inicial
echo "[ronda3] Ejecutando backtest con ${HIST_DATA} (balance inicial: $INITIAL_BALANCE)"
...
if [ -n "$DEMO_LOG" ] && [ -n "$BACKTEST_LOG" ]; then
  echo "[ronda3] Demo log: $DEMO_LOG"
  echo "[ronda3] Backtest log: $BACKTEST_LOG"
  python tests/validation/compare_results.py --testing "$DEMO_LOG" --backtest "$BACKTEST_LOG" --output ${COMPARISON_LOG} || echo "[ronda3] compare script devolvió error"
else
  echo "[ronda3] No se encontraron logs para comparar (Demo: ${DEMO_LOG:-None}, Backtest: ${BACKTEST_LOG:-None})"
fi

echo "[ronda3] Finalizado. Revisa ${COMPARISON_LOG} y los logs en logs/."
