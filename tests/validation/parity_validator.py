...
def compare_parity(
    demo_db: str, backtest_db: str, start_ts: float = None, end_ts: float = None, tolerance: float = 0.005
):
    logger.info("=" * 60)
    ...
    if not demo_trades.empty and start_ts and end_ts:
        # Phase 1800: Prioritize pure numeric columns (Unix Epoch)
        if "t0_signal_ts" in demo_trades.columns:
            demo_trades["ts_numeric"] = pd.to_numeric(demo_trades["t0_signal_ts"], errors="coerce")
        elif "t4_fill_ts" in demo_trades.columns:
            demo_trades["ts_numeric"] = pd.to_numeric(demo_trades["t4_fill_ts"], errors="coerce")
        else:
            demo_trades["ts_numeric"] = pd.to_numeric(demo_trades["timestamp"], errors="coerce")

        # Last resort: ISO conversion if everything else is NaN
        if demo_trades["ts_numeric"].isna().all():
            demo_trades["ts_numeric"] = pd.to_datetime(demo_trades["timestamp"], errors="coerce").astype(int) / 10**9

        demo_trades = demo_trades[
            (demo_trades["ts_numeric"] >= start_ts - 10) & (demo_trades["ts_numeric"] <= end_ts + 10)
        ].copy()

    if not bt_trades.empty and start_ts and end_ts:
        if "t0_signal_ts" in bt_trades.columns:
            bt_trades["ts_numeric"] = pd.to_numeric(bt_trades["t0_signal_ts"], errors="coerce")
        elif "t4_fill_ts" in bt_trades.columns:
            bt_trades["ts_numeric"] = pd.to_numeric(bt_trades["t4_fill_ts"], errors="coerce")
        else:
            bt_trades["ts_numeric"] = pd.to_numeric(bt_trades["timestamp"], errors="coerce")

        if bt_trades["ts_numeric"].isna().all():
            bt_trades["ts_numeric"] = pd.to_datetime(bt_trades["timestamp"], errors="coerce").astype(int) / 10**9

        bt_trades = bt_trades[
            (bt_trades["ts_numeric"] >= start_ts - 10) & (bt_trades["ts_numeric"] <= end_ts + 10)
        ].copy()
    ...
    for i, d_row in demo_trades.iterrows():
        ...
        match = None
        min_diff = 120.0  # Max 120s difference (demo vs backtest have signal timing drift)
        ...
        if match:
            ...
            logger.info(
                f"   {status} | Demo Entry: {d_entry:.4f} vs BT Entry: {b_entry:.4f} (Diff: {slip_pct:.3f}%) | PnL Leak: {pnl_diff:+.4f}"
            )
        else:
    ...
    if len(misses_bt) > 0:
        logger.error(
            f"   ❌ Backtest executed {len(misses_bt)} trades NOT seen in Demo (Simulation False Positive - Liquidity/Latency Limit)."
        )
