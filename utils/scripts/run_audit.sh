...
.venv/bin/python main.py --run-type trade --mode demo --symbol LTC/USDT:USDT --timeout 15 --fast-track --close-on-exit > logs/demo_exec.log 2>&1

echo "Phase 2: Running Simulator Replay..."
.venv/bin/python backtest.py --run-type trade --data data/raw/LTCUSDT_trades_1week.csv --symbol LTC/USDT:USDT --limit 5000 --fast-track > logs/bt_exec.log 2>&1
