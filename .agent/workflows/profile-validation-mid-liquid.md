---
description: Profile Validation Protocol (MID_LIQUID Profile — ADA × ARB)
---
...
## Overview
Validates that the coin profile system works correctly for the MID_LIQUID profile using its constituent assets (ADA, ARB) across multiple market conditions.
Runs backtests across distinct conditions using Set A and validates:
- Profile assignment correctness (MID_LIQUID via 4-dim institutional clustering)
...
|-------|--------|---------|-----|
| **ADA** | ADA/USDT:USDT | MID_LIQUID | Book density muy alto |
| **ARB** | ARB/USDT:USDT | MID_LIQUID | Book density muy alto |

**Market Conditions & Datasets (Set A):**
| Asset | Datasets | Protocol |
|-------|----------|----------|
| **ADA** | 2024-11-01_ADAUSDT.db, 2025-02-01_ADAUSDT.db, 2025-03-01_ADAUSDT.db, 2025-07-01_ADAUSDT.db, 2025-11-01_ADAUSDT.db, 2026-05-01_ADAUSDT.db | `set_a_ada` |
| **ARB** | 2024-11-01_ARBUSDT.db, 2025-02-01_ARBUSDT.db, 2025-05-01_ARBUSDT.db, 2025-06-01_ARBUSDT.db, 2025-10-01_ARBUSDT.db, 2026-04-01_ARBUSDT.db | `set_a_arb` |
...
```bash
echo "=== MID_LIQUID Set A Datasets ==="
for f in \
  data/datasets/backtest_ready/2024-11-01_ADAUSDT.db \
  data/datasets/backtest_ready/2025-02-01_ADAUSDT.db \
  data/datasets/backtest_ready/2025-03-01_ADAUSDT.db \
  data/datasets/backtest_ready/2025-07-01_ADAUSDT.db \
  data/datasets/backtest_ready/2025-11-01_ADAUSDT.db \
  data/datasets/backtest_ready/2026-05-01_ADAUSDT.db \
  data/datasets/backtest_ready/2024-11-01_ARBUSDT.db \
  data/datasets/backtest_ready/2025-02-01_ARBUSDT.db \
  data/datasets/backtest_ready/2025-05-01_ARBUSDT.db \
  data/datasets/backtest_ready/2025-06-01_ARBUSDT.db \
  data/datasets/backtest_ready/2025-10-01_ARBUSDT.db \
  data/datasets/backtest_ready/2026-04-01_ARBUSDT.db; do
...
>
> **① ADA — set_a_ada** (6 datasets, ~3 workers)
> ```bash
> PYTHONUNBUFFERED=1 .venv/bin/python scripts/orchestrator.py --protocol set_a_ada \
>   > logs/orchestrator_ada.log 2>&1
> ```
>
> **② ARB — set_a_arb** (6 datasets, ~3 workers)
> ```bash
> PYTHONUNBUFFERED=1 .venv/bin/python scripts/orchestrator.py --protocol set_a_arb \
>   > logs/orchestrator_arb.log 2>&1
> ```
>
> **③ Merge final**
> ```bash
...
**What to look for:**
- ¿ADAUSDT, ARBUSDT → asignado a MID_LIQUID?
...
|----------|---------------|
| ¿ADA/ARB → MID_LIQUID? | Step 4 (post-audit) |
...
|-------|-----------|---------------|-------------|----------|---------|
| ADA | BALANCE | ✅ | Expectancy > 0.12% | High Wall > Thin Wall | **CERTIFIED** |
| ARB | BALANCE | ✅ | Expectancy > 0.12% | — | **WATCH** |
...
## Appendix: MID_LIQUID Profile (current)

4-dim institutional microstructure (per `config/clusters_fixed.json`):
```python
"tick_size_efficiency": 0.50   # How fast spread clears
"book_density": 22.0          # Total volume / spread
"volume_vol_ratio": 7.0       # Energy to move price
"speed": 4.0                  # Trades per second
```
...
TREND_UP:   TP 1.2% / SL 2.5%
TREND_DOWN: TP 2.0% / SL 3.0%
BALANCE:    TP 0.8% / SL 2.5%
...
**Membership**: ADA, ARB (from clusters_fixed.json).
