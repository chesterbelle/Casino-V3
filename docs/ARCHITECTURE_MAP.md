...
```mermaid
graph TD
    A[Tick: Price + Qty + Ts] --> B[PressureEngine]
    B --> C{¿Es absorción instantánea?}
    C -->|SÍ| D[TacticalAbsorption Detector]
    C -->|NO| E[ScenarioManager / SignalArbitrator]

    D --> F[SetupEngine]
    E --> G{¿Failed Breakout?}
    G -->|SÍ| H[FailedBreakout Detector]
    G -->|NO| I{¿Liquidity Exhaustion?}
    I -->|SÍ| J[LiquidityExhaustion Detector]
    I -->|NO| K{¿Trend Acceptance?}
    K -->|SÍ| L[TrendAcceptance Detector]
...
1.  Abre `decision/scenarios/trend_acceptance.py`.
...
|---------|----------|-----------------|------------------|
| **MEGA_LIQUID** | ADA, ARB, NEAR | Libros gruesos, bajo spread | `z_score_min=3.0`, `noise_max=0.25` |
...
- **2026-06-27:** Creado `ARCHITECTURE_MAP.md` para documentar flujo real.
...
> **"Si no puedes encontrar el parámetro que buscas en este mapa, es porque no está documentado. Ábrelo, encuéntralo, y agrégalo para el próximo."**
